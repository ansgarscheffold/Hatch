...
class XTerminalCore: XTerminal {
    ...
    func setTerminalFontSize(with size: Int) {
        DispatchQueue.global().async {
            ...
            self.evaluateFitToHostViewOnBackgroundThread()
        }
    }

    /// Passt xterm an die aktuelle WebView-Größe an (WKWebView: kein zuverlässiges vh/vw-Resize).
    func fitToHostView() {
        guard associatedWebDelegate.navigateCompleted else { return }
        let runFit = {
            self.associatedWebView.evaluateJavaScript(Self.fitToHostJavaScript, completionHandler: nil)
        }
        if Thread.isMainThread {
            runFit()
        } else {
            DispatchQueue.main.async(execute: runFit)
        }
    }

    func evaluateFitToHostViewOnBackgroundThread() {
        DispatchQueue.global().async {
            self.associatedWebView.evaluateJavascriptWithRetry(javascript: Self.fitToHostJavaScript)
        }
    }

    static let fitToHostJavaScript = """
    (function() {
        try {
